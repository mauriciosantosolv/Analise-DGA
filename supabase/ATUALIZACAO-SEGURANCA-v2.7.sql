-- CliqueObras v2.7
-- Hardening de perfis, convites, permissões, hierarquia e módulos RDO/HH.
    ...
    and exists (
      ...
      where actor.organization_id=target_org
        ...
        and (
          ...
          or (
            (
              (actor.role='editor' and target_role in ('editor','viewer'))
              or (actor.role='viewer' and target_role='viewer')
            )
            and coalesce((actor.permissions->>'manage_users')::boolean,false)
            and not coalesce((target_permissions->>'manage_users')::boolean,false)
            and not exists (
              select 1
              from jsonb_array_elements_text(
                coalesce(target_permissions->'view','[]'::jsonb)
              ) as requested(value)
              where not (
                coalesce(actor.permissions->'view','[]'::jsonb) ? requested.value
              )
            )
            and not exists (
              select 1
              from jsonb_array_elements_text(
                coalesce(target_permissions->'edit','[]'::jsonb)
              ) as requested(value)
              where not (
                coalesce(actor.permissions->'edit','[]'::jsonb) ? requested.value
              )
            )
            and not exists (
              select 1
              from jsonb_array_elements(
                coalesce(target_permissions->'rdo_projects','[]'::jsonb)
              ) as requested(item)
              where not exists (
                select 1
                from jsonb_array_elements(
                  coalesce(actor.permissions->'rdo_projects','[]'::jsonb)
                ) as allowed(item)
                where allowed.item->>'id'=requested.item->>'id'
              )
            )
          )
        )
