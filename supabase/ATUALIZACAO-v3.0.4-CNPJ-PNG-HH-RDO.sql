-- CliqueObras v3.0.4
-- CNPJ da contratada, papel timbrado JPG/PNG e preferências de função/status HH.
...
begin
  ...
  if target.store='settings'
    and target.record_id=any(array[
      'companyName','companyCnpj','companyLogo','pdfLetterhead','rdoShiftStart','rdoShiftEnd',
  ...
  if new.store='settings' then
    ...
    if new.record_id='companyCnpj'
      and (jsonb_typeof(setting_value)<>'string'
        or coalesce(setting_value#>>'{}','') !~ '^$|^[0-9]{2}\.[0-9]{3}\.[0-9]{3}/[0-9]{4}-[0-9]{2}$') then
      raise exception 'CNPJ da empresa inválido.';
    end if;
    ...
    if new.record_id='pdfLetterhead' then
      ...
      if length(image_value)>2500000
        or (image_value<>'' and image_value !~* '^data:image/(jpeg|png);base64,[a-z0-9+/=]+$') then
  ...
  if new.store='labor_rates' and coalesce(new.data->>'isBaseCost','false')<>'true' then
    role_mode := coalesce(new.data->>'roleDisplayMode','client');
    if role_mode not in ('client','internal')
      or length(coalesce(new.data->>'commercialRole',''))>140
      or (new.data ? 'active' and coalesce(new.data->>'active','') not in ('true','false')) then
      raise exception 'Configuração de valor HH inválida.';
    end if;
  end if;
