...
create or replace function clique_obras_private.validate_v3084_records()
returns trigger
language plpgsql
security invoker
set search_path=''
...
revoke all on function clique_obras_private.validate_v3084_records()
  from public,anon,authenticated;

-- Evita duas validações quando a 3.0.8.3 já tiver sido instalada.
drop trigger if exists cliqueobras_validate_v3083_records on public.app_records;
drop trigger if exists cliqueobras_validate_v3084_records on public.app_records;
create trigger cliqueobras_validate_v3084_records
before insert or update or delete on public.app_records
for each row execute function clique_obras_private.validate_v3084_records();

create or replace function public.clique_obras_apply_omie_entries_v3084(
...
revoke all on function public.clique_obras_apply_omie_entries_v3084(uuid,uuid,jsonb,uuid)
  from public,anon,authenticated;
grant execute on function public.clique_obras_apply_omie_entries_v3084(uuid,uuid,jsonb,uuid)
  to service_role;

comment on function public.clique_obras_apply_omie_entries_v3084(uuid,uuid,jsonb,uuid)
