...
Views.medicoes = {
  ...
  render(){
    ...
    const filteredMeasurements=State.measurements.filter(m=>!State.filters.project || m.projectId===State.filters.project);
    const totalMeasured = filteredMeasurements.reduce((s,m)=>s+m.value,0);
    const totalInvoiced = filteredMeasurements.filter(m=>U.norm(m.status).startsWith('faturad')).reduce((s,m)=>s+m.value,0);
    const totalApproved = filteredMeasurements.filter(m=>U.norm(m.status).startsWith('aprova')).reduce((s,m)=>s+m.value,0);
    const totalAwaiting = filteredMeasurements.filter(m=>U.norm(m.status)==='aguardando aprovacao').reduce((s,m)=>s+m.value,0);
    ...
    $c().innerHTML = `
      ...
      ${Object.keys(byProj).length ? Object.entries(byProj).map(([pid, items]) => {
        ...
        const measured = items.reduce((s,m)=>s+m.value,0);
        const invoiced = items.filter(m=>U.norm(m.status).startsWith('faturad')).reduce((s,m)=>s+m.value,0);
        const approved = items.filter(m=>U.norm(m.status).startsWith('aprova')).reduce((s,m)=>s+m.value,0);
        const awaiting = items.filter(m=>U.norm(m.status)==='aguardando aprovacao').reduce((s,m)=>s+m.value,0);
        const pctM = p && p.saleValue>0 ? measured/p.saleValue*100 : null;
        return `<div class="card" style="margin-bottom:12px">
          <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:8px">
            <h3>${U.esc(U.projLabel(p))}</h3>
            <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap"><b style="color:var(--green)">${U.money(invoiced)} faturado</b><span class="tag tag-blue">${U.money(approved)} aprovado</span><span class="tag tag-amber">${U.money(awaiting)} aguardando aprovação</span><span class="tag ${pctM!=null&&pctM>=100?'tag-green':'tag-blue'}">${U.pct(pctM)} medido</span></div></div>
