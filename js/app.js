...
const App = {
  ...
  applyNavigationPermissions(){
    document.querySelectorAll('.nav-item[data-view]').forEach(btn=>{
      btn.hidden=!this.canOpenView(btn.dataset.view);
    });
  },
  addReadOnlyBanner(){
    if(typeof Cloud==='undefined' || !Cloud.active()) return;
    const store=this.primaryStore();
    if(!store || Cloud.canEditStore(store)) return;
    const content=document.getElementById('content');
    if(!content || content.querySelector('.permission-banner')) return;
    const banner=document.createElement('div');
    banner.className='permission-banner';
    banner.innerHTML='<i data-lucide="eye"></i><span><b>Acesso para consulta.</b> Você pode visualizar este módulo, mas as alterações estão bloqueadas pelas permissões da sua organização.</span>';
    content.prepend(banner); U.icons();
  },
  guardReadOnlyAction(event){
    if(typeof Cloud==='undefined' || !Cloud.active()) return;
    const store=this.primaryStore();
    if(!store || Cloud.canEditStore(store)) return;
    const target=event.target.closest('[onclick],button');
    if(!target) return;
    ...
    UI.toast('Seu acesso a este módulo é somente para consulta.','warn',5500);
  },
  ...
  applyStorageStatus(){
    ...
    if(typeof Cloud!=='undefined' && Cloud.active()){
      ...
      el.textContent=`v4.2.18 · ${org?org.name:'nuvem conectada'}${pending?` · ${pending} pendente(s)`:''}`;
    }else el.textContent='v4.2.18 · dados locais';
  },
  showCloudLogin(){
  ...
  async init(){
   try{
    ...
    await State.reload();
    // v4.2.18 - jornada padrao para o encarregado, que nao le o store settings
    try{ if(typeof RDO!=='undefined'&&typeof RDO.loadRemoteShiftDefaults==='function') await RDO.loadRemoteShiftDefaults(); }catch(error){}
